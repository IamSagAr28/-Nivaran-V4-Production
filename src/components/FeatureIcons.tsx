        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {features.map((feature) => (
            <div key={feature.title} className="text-center space-y-2 feature-item-group">
              ...
              <h3 className="font-bold text-base" style={{ color: '#4a3b2c', fontSize: '1.05rem', fontWeight: 'bold' }}>{feature.title}</h3>
              <p className="text-sm" style={{ color: '#4a3b2c', fontSize: '0.9rem', lineHeight: 1.4 }}>{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
